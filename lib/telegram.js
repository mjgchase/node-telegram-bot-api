...
var _telegramWebHook = require('./telegramWebHook.js');

var _telegramWebHook2 = _interopRequireDefault(_telegramWebHook);

var _telegramPolling = require('./telegramPolling.js');

var _telegramPolling2 = _interopRequireDefault(_telegramPolling);
...
var _request2 = require('@cypress/request');
...
// shims
_arrayPrototype2.default.shim(); // for Node.js v0.x
var debug = (0, _debug2.default)('node-telegram-bot-api');
var deprecate = { deprecate: _utils.deprecate }.deprecate;
...
var TelegramBot = function (_EventEmitter) {
  ...
  _createClass(TelegramBot, [{
    ...
    /**
     * Add a listener for the specified [event](https://github.com/yagop/node-telegram-bot-api/blob/master/doc/usage.md#events).
    ...
    /**
     * Both request methods to obtain messages are implemented. To use standard polling, set `polling: true`
    ...
    /**
     * The types of a message updates the library handles.
     * @type {String[]}
     */
  ...
  _createClass(TelegramBot, [{
    ...
    value: function _request(_path) {
      ...
      return fetch(options).then(function (resp) {
        var data = void 0;
        try {
    ...
    value: function _formatSendData(type, data) {
      ...
      if (data instanceof _stream2.default.Stream) {
        if (!filename && data.path) {
          ...
          var url = _url2.default.parse(_path3.default.basename(data.path.toString()));
          if (url.pathname) {
            filename = _querystring2.default.unescape(url.pathname);
          }
        }
        ...
        if (!contentType) {
          var filetype = (0, _fileType2.default)(data);
          if (filetype) {
        ...
        if (this.options.filepath && _fs2.default.existsSync(data)) {
          filedata = _fs2.default.createReadStream(data);
          if (!filename) {
            filename = _path3.default.basename(data);
          }
        } else {
          return [null, data];
        }
      ...
      contentType = contentType || _mime2.default.lookup(filename);
    ...
    value: function _formatSendMultipleData(type, files) {
      ...
      files.forEach(function (file, index) {
        ...
        if (filedata instanceof _stream2.default.Stream) {
          if (!filename && filedata.path) {
            var url = _url2.default.parse(_path3.default.basename(filedata.path.toString()), true);
            if (url.pathname) {
              filename = _querystring2.default.unescape(url.pathname);
            }
          }
          ...
          if (!contentType) {
            var filetype = (0, _fileType2.default)(filedata);
            if (filetype) {
              contentType = filetype.mime;
              var ext = filetype.ext;
              if (ext) {
        ...
        } else if (_fs2.default.existsSync(filedata)) {
          filedata = _fs2.default.createReadStream(filedata);
          if (!filename) {
            filename = _path3.default.basename(filedata.path);
          }
        } else {
    ...
    value: function startPolling() {
      ...
      if (!this._polling) {
        this._polling = new _telegramPolling2.default(this);
      }
     ...
     * Use this method to get link for file for later use.
    ...
    /**
     * Return a readable stream for a file.
     *
     ...
     * which returns the full URI to the file on the remote server.
     *
     * @param {String} fileId File identifier to get info about
     * @param {Object} [form] Additional Telegram query options
     * @return {module:stream.internal.PassThrough} fileStream
     */
    ...
    value: function getFileStream(fileId) {
      ...
      var fileStream = new _stream2.default.PassThrough();
      fileStream.path = fileId;
      this.getFileLink(fileId, form).then(function (fileURI) {
        fileStream.emit('info', {
          uri: fileURI
        });
        (0, _pump2.default)((0, _request3.default)(Object.assign({ uri: fileURI }, _this3.options.request)), fileStream);
      }).catch(function (error) {
    ...
    /**
     * Downloads a file in the specified folder.
     *
    ...
    value: function downloadFile(fileId, downloadDir) {
      ...
      fileStream.on('info', function (info) {
        ...
        var filePath = _path3.default.join(downloadDir, fileName);
        (0, _pump2.default)(fileStream, _fs2.default.createWriteStream(filePath), function (error) {
          if (error) {
            return reject(error);
          }
     ...
     * the `msg` and the result of executing `regexp.exec` on a message text.
     */
    ...
    value: function openWebHook() {
      ...
      if (!this._webHook) {
        this._webHook = new _telegramWebHook2.default(this);
      }
    ...
    value: function setWebHook(url) {
      ...
      if ((typeof options === 'undefined' ? 'undefined' : _typeof(options)) !== 'object' || options instanceof _stream2.default.Stream) {
     ...
     * @param {Object} [form] Additional Telegram query options
     * @return {Promise} On success, a sent message(https://core.telegram.org/bots/api#message) object is returned
     * @see https://core.telegram.org/bots/api#sendmessage
     */
     ...
     * @param {Object} options Additional Telegram query options
     * @return {Promise} On success, an array of a sent message (https://core.telegram.org/bots/api#message)
     * is returned.
    ...
    value: function answerCallbackQuery(callbackQueryId) {
      ...
      /* The older method signature (in/before v0.29.0) was answerCallbackQuery([form]).
           * We need to ensure backwards-compatibility while maintaining
           * consistency of the method signatures throughout the library. */
      if ((typeof callbackQueryId === 'undefined' ? 'undefined' : _typeof(callbackQueryId)) === 'object') {
        /* eslint-disable no-param-reassign, prefer-rest-params */
        deprecate('The method signature answerCallbackQuery([form]) has been deprecated since v0.29.0');
     ...
     * @param {Object} [form] Additional Telegram query options
     * @param {Object} [fileOptions] Optional file related meta-data
     * @return {Promise} On success, a sent message(https://core.telegram.org/bots/api#message) is returned
